...
if (!isset ($_GET['view']) && !isset($_GET['join']))
{
    ...
    define("CLAN_INFO", "<p><i>Domy klanów</i></p>");
...
if (isset ($_GET['view']) && $_GET['view'] == 'my')
{
    ...
    if (isset ($_GET['step']) && $_GET['step'] == 'owner')
    {
        ...
        if (isset ($_GET['step2']) && $_GET['step2'] == 'rank')
        {
            ...
            define('NO_RANKS2', 'Na razie nie posiadasz określonych rang i tagów w klanie! Tutaj właśnie możesz je stworzyć. Możesz je wpisywać w dowolnej kolejności, dodatkowo nie musisz wypełniać wszystkich pól (możesz na przykład zrobić tylko 3 rangi z tagami).<br/>Uwaga! Prefix i sufix tagu klanowego mogą mieć łącznie nie więcej niż 9 znaków.');
            define("RANK", "ranga");
			define ('TAG_PREFIX', 'prefix tagu');
			define ('TAG_SUFIX', 'Wspólny sufix tagu');

            define("EDIT_RANKS", "W tym miejscu możesz edytować lub dodawać nowe rangi. Możesz też określić odpowiednie dla poszczególnych rang tagi klanowe. Główny tag (sufix) razem z tagiem dodatkowym, zależnym od rangi może mieć maksymalnie 9 znaków. Pamiętaj, że <b>spacja także liczy się jako znak</b>!");
        ...
        if (isset ($_GET['step2']) && $_GET['step2'] == 'walka')
        {
            ...
            define("ENEMY_WIN2", "wykrzyknął komende do jego towarzyszy. Ruszasz razem z twoimi podwładnymi i zaczyna sie bitwa. Atakujesz przywodcę klanu");
            define("ENEMY_WIN3", "i podczas walki giniesz. Przegraliście tą walkę...  Z waszego skarbca przeciwnik wyniósł");
