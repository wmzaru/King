...
require_once("languages/".$player -> lang."/quest15.php");

$objAction = $db -> Execute("SELECT `action` FROM `questaction` WHERE `player`=".$player -> id." AND `quest`=15");
$objQuest = new Quests('grid.php', 15, $objAction -> fields['action']);
...
{
    $xx = rand(1,5);
    $db -> Execute("UPDATE players SET platinum=platinum + $xx WHERE id=".$player -> id);
    $smarty -> assign("Text", "<br />Otrzymujesz $xx mithrillu.");
    $objQuest -> Finish(0, 'Altara', 'Labirynt', 'grid.php');
}
