...
if ($objAction -> fields['action'] == '1')
{
    $xx = rand(5,9);
    $xxx = rand(80,120);
    if ($player -> strength + $player -> agility >= $xxx)
    {
        $objQuest -> Gainexp($xx);
        $objQuest -> Show('2.1');
        $smarty -> assign("Link", "<br /><br />(<a href=\"grid.php?step=quest\">dalej</a>)");
...
if ($objAction -> fields['action'] == '2.1.1')
{
     $objQuest -> Show('2.1.2');
     $objQuest -> Finish(0,'Altara','Labirynt', 'grid.php');
}
if ($objAction -> fields['action'] == '2.2.1')
{
    $objQuest -> Show('2.2.2');
    $smarty -> assign("Link", "<br /><br />(<a href=\"grid.php?step=quest\">dalej</a>)");
}

if ($objAction -> fields['action'] == '2.2.2')
{
    $xx = rand(20,50)/100 * $player -> hp;
    $db -> Execute("UPDATE players SET hp=hp - $xx WHERE id=".$player -> id);
    $smarty -> assign("Text", "<br /> Tracisz " .$xx." punktów życia.");
    $objQuest -> Finish(0,'Altara','Labirynt', 'grid.php');
}
