...
$objAction = $db -> Execute("SELECT `action` FROM `questaction` WHERE `player`=".$player -> id." AND `quest`=14");
$objQuest = new Quests('grid.php', 14, $objAction -> fields['action']);
...
{
    $xx = rand(5,10)/100 * $player -> max_hp;
    $xx = round($xx);
    $db -> Execute("UPDATE players SET hp=hp - $xx WHERE id=".$player -> id);
    $smarty -> assign("Text", "<br />Straciłeś $xx punktów życia.");
